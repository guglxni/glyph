...
fn provider() -> Arc<dyn TeeProvider> {
    ...
    std::env::set_var(
        ...
        "audit-log-test-passphrase-32bytes-min!!",
    );
...
fn append_and_verify_chain() {
    ...
    log.append(make_inner(1), 1_700_000_000, prov.as_ref())
        .unwrap();
    log.append(make_inner(2), 1_700_000_001, prov.as_ref())
        .unwrap();
    log.append(make_inner(3), 1_700_000_002, prov.as_ref())
        .unwrap();
    log.verify_chain(prov.as_ref())
...
fn persisted_chain_replays_after_restart() {
    ...
    log.verify_chain(prov.as_ref())
        .expect("chain on disk must verify");
}
